...
function SlotMachine(container, reels, callback, options) {
  ...
  function spinReels() {
    ...
    tempArray = [];
    reels.forEach(reel => {
      //if (cheatMode == true) {const selected = "seven"} else {const selected = selectRandSymbol(reel.symbols)};
  ...
  function playSound(url) {
    if (url) {
      ...
      audio.play();
      //clearInterval(audioInterval);
      //audioInterval = setInterval(function(){if (shouldPlay == true) {audio.play()}},audio.duration);  
    }
  }
  ...
  /*function spinLoop() {
    if (document.querySelectorAll(".strip")[0].classList.contains("spin") || document.querySelectorAll(".strip")[1].classList.contains("spin") || document.querySelectorAll(".strip")[2].classList.contains("spin") || document.querySelectorAll(".strip")[3].classList.contains("spin") || document.querySelectorAll(".strip")[4].classList.contains("spin")) {
      (new Audio('./sounds/reelsBegin1.mp3')).play();
    }
  }
  window.setTimeout(function(){window.setInterval(spinLoop,1222)},600);*/
  function spinLoop() {
